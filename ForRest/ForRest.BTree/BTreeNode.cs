// --------------------------------------------------------------------------------------------------------------------
// <copyright file="BTreeNode.cs" company="Warsaw University of Technology">
//   
// </copyright>
// <summary>
//   The b tree node.
...
namespace ForRest.BTree
{
    ...
    public class BTreeNode<T> : Node<T>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="BTreeNode{T}"/> class. 
        /// Constructor.
        /// </summary>
        /// <param name="degree">
        /// The degree.
        ...
        public override Node<T> Parent
        {
            ...
            set
            {
                this._parent = (BTreeNode<T>)value;
            }
        }
        ...
        {
            return (BTreeNode<T>)this.Neighbors[i];
        }
        ...
        public BTreeNode<T> Delete(T data, int index)
        {
            ...
            for (int i = 0; i < this.Values.Count; i++)
            {
                ...
                if (result <= 0)
                {
                    successorNode = (BTreeNode<T>)this.Neighbors[i + 1];
                }
            }
        ...
        public BTreeNode<T> Merge()
        {
            ...
            for (int i = 0; i < this._parent.Neighbors.Count; i++)
            {
                if (this._parent.Neighbors[i] == this)
                {
                    ...
                    if (i > 0)
                    {
                        left = (BTreeNode<T>)this._parent.Neighbors[i - 1];
                    }

                    if (i + 1 < this._parent.Neighbors.Count)
                    {
                        right = (BTreeNode<T>)this._parent.Neighbors[i + 1];
                    }
                }
            }
        ...
        public BTreeNode<T> Split()
        {
            ...
            this.Values.Clear();
            var leftNode = new BTreeNode<T>(this._m, (BTreeNode<T>)this.Parent, leftData, leftNeighbors)
                {
                   IsLeaf = false 
                };
            var rightNode = new BTreeNode<T>(this._m, (BTreeNode<T>)this.Parent, rightData, rightNeighbors)
                {
                   IsLeaf = false 
                };
            ...
            var centerNode = new BTreeNode<T>(this._m, null, centerDataList, children) { IsLeaf = false };
        ...
        public BTreeNode<T> Split(T data)
        {
            ...
            this.Values.Clear();
            var leftNode = new BTreeNode<T>(this._m, (BTreeNode<T>)this.Parent, leftData);
            var rightNode = new BTreeNode<T>(this._m, (BTreeNode<T>)this.Parent, rightData);
